...
// 		.text(
// 			numberWithSeparator(
// 				filteredProducts[0].unitPrice * (1 - filteredProducts[0].discount),
// 				','
// 			) + ' VND'
// 		);
	...
	for (let i = 0; i < Math.min(8, products.length); i++) {
		$(`.listShow`).append(`
			<div class="item">
				...
				<a href="./Detail/index.html?id=${
					products[i].id
				}" style="display: inline-block" class="name-item">${products[i].name}</a>
				<div class="price-item">${
					numberWithSeparator(products[i].price, ',') + ' VND'
				}</div>
			</div>
	`);
	}
...
// // show category in main
// function showCategoryList() {
// 	console.log('showCategoryList');
// 	$.ajax({
// 		url: 'http://localhost/WebProject/api/category',
// 		type: 'GET',
// 		success: function (data) {
// 			var categoryList = $('.categoryList');
// 			categoryList.empty(); // Clear the existing items

// 			data.forEach(function (item) {
// 				var categoryItem = $('<div>').addClass('categoryItem');
// 				var imageCategory = $('<div>').addClass('image-category');
// 				var img = $('<img>').attr('src', item.image).css({
// 					width: '100%',
// 					height: '100%',
// 					objectFit: 'cover',
// 				});
// 				var titleCategory = $('<div>')
// 					.addClass('title-category')
// 					.text(item.name);

// 				imageCategory.append(img);
// 				categoryItem.append(imageCategory).append(titleCategory);
...
function showCategoryList() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			data.forEach(function (item) {
				var categoryItem = $('<div>').addClass('categoryItem');
				var categoryItem = $('<div>').addClass('categoryItem clickable');
				var imageCategory = $('<div>').addClass('image-category');
				var img = $('<img>').attr('src', item.image).css({
					width: '100%',
					height: '100%',
				...
				// Add click event to navigate to the Classify page with the category ID
				categoryItem.click(function () {
					window.location.href =
						'http://localhost/client/Classify/index.html?categoryId=' + item.id;
				});
			});
		},
		error: function (error) {
